...
public class Cp implements Command {
  ...
  public String run(FileManager fm, String[] args) throws NonvalidArgumentException {
    ...
    if (fm.checkPathToDir(currentdir, args[1])) {
      ...
      if (fm.checkPathToFile(args[2], currentdir)) {
    	  throw new NonvalidArgumentException("error, can't copy dir to file \n");
      }
      ...
      moveDir(fm, oldPathDir, newPathDir);
      return "";
     // check if OLDPath is a valid file
      ...
      //check if NEWpath is a valid dir, if not create new file and copy oldFile to Newpath
      if (!fm.checkPathToDir(currentdir, args[2])) {
   ...
   * @throws NonvalidArgumentException the nonvalid argument exception
   */
  private void moveDir(FileManager fm, Directory oldPathDir, Directory newPathDir)
      throws NonvalidArgumentException {
